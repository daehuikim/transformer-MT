...
import torch
import sys
sys.path.append("./transformer-MT/ST")
from prepro import preprocess_dataset

# 데이터셋 로드
# dataset = load_dataset("patrickvonplaten/librispeech_asr_dummy", "clean")
dataset_local = preprocess_dataset('./tst-COMMON-copy','./ST-output')
...
for data in dataset_local["validation"]["wave_info"]:
    transcription = transcribe_audio(data)
    pred_f.write(transcription + "\n")
    print("predicted utterances:",transcription)
